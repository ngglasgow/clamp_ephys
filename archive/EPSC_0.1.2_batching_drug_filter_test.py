...
# use os.path.join(parent_dir, child_dir, next_child_dir, etc)
home_dir = os.path.expanduser('~')
project_dir = os.path.join(home_dir, 'ngglasgow@gmail.com', 'Data_Urban', 'jane', 'OSN Stim Analysis')
figure_dir = os.path.join(project_dir, 'analysis', 'figures')
table_dir = os.path.join(project_dir, 'analysis', 'tables')
data_dir = os.path.join(project_dir, 'ETC', 'Data')

''' ###################### For remote server file system ################## '''
# these are OS specific and therefore will need to be built and run to your machine

# on mac
data_dir = '/Volumes/Urban/Huang'

data_dir = '/run/user/1000/gvfs/smb-share:server=130.49.237.41,share=urban/Huang'
...
if math.isnan(exclude_start) is False:
    ...
    # else only exclude traces in between start and stop
        ...
        end = data.iloc[:, int(exlcude_stop):]
...
# filter signal with butterworth filter at 1 kHz for data
filt_data=None
filt_data = elephant.signal_processing.butter(data,
                                              lowpass_freq=1000.0,
                                              fs=10000.0)
filt_data = pd.DataFrame(filt_data)

filt_baseline = mean_baseline(filt_data, 500)
filt_peaks = epsc_peak(filt_data, filt_baseline, 500)

filt_1k_data = elephant.signal_processing.butter(data.T,
                                              lowpass_freq=1000.0,
                                              fs=10000.0)
filt_1k_data = pd.DataFrame(filt_1k_data).T
filt_1k_baseline = mean_baseline(filt_1k_data, 500)
filt_1k_peaks = epsc_peak(filt_data, filt_1k_baseline, 500)


filt_500_data = elephant.signal_processing.butter(data.T,
                                              lowpass_freq=500.0,
                                              fs=10000.0)
filt_500_data = pd.DataFrame(filt_500_data).T
filt_500_baseline = mean_baseline(filt_500_data, 500)
filt_500_peaks = epsc_peak(filt_500_data, filt_500_baseline, 500)


filt_200_data = elephant.signal_processing.butter(data.T,
                                              lowpass_freq=200.0,
                                              fs=10000.0)
filt_200_data = pd.DataFrame(filt_200_data).T
filt_200_baseline = mean_baseline(filt_200_data, 500)
filt_200_peaks = epsc_peak(filt_200_data, filt_200_baseline, 500)


filt_data_df = pd.DataFrame()
for i in range(len(data.columns)):
    filt_data = elephant.signal_processing.butter(data[i],
                                                  lowpass_freq=1000.0,
                                                  fs=10000.0)
    filt_data = pd.DataFrame(filt_data)
    filt_data_df = pd.concat([filt_data_df, filt_data], axis=1, ignore_index=True)

filt_baseline = mean_baseline(filt_data_df, 500)
filt_peaks = epsc_peak(filt_data_df, filt_baseline, 500)

plt.figure()
plt.plot(data[42])
plt.plot(filt_1k_data[42])
plt.plot(filt_500_data[42])
plt.plot(filt_200_data[42])

plt.figure()
plt.plot(peaks, marker='.')
# plt.plot(filt_1k_peaks, marker='.')
plt.plot(filt_500_peaks, marker='.')
plt.plot(filt_200_peaks, marker='.')
...
fig
# save figure to file
fig_save_path = os.path.join(figure_dir, cell_id)
...
data_dict['Filtered Peaks (nA)'] = filt_500_peaks
...
sweep_meta_path = os.path.join(table_dir, cell_id)
sweep_meta_data.to_csv(sweep_path + '_all_sweeps_data.csv', float_format='%8.4f', index=False, header=True)
...
summary_path = os.path.join(table_dir, cell_id)
summary_data.to_csv(mean_path + '_summary_data.csv', float_format='%8.4f', index=False)

''' Make into autogenerate a jupyter notebook? '''
''' separate into functions and code '''
