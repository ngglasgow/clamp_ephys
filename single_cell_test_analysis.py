...
data = clamp_ephys.workflows.cell(r"C:\Users\jhuang\Documents\phd_projects\Injected_GC_data\VC_pairs\data\p2\JH200303_c1_spontaneous.ibw", fs=fs, path_to_data_notes=data_path, timepoint='p2', amp_factor=amp_factor)

data.get_raw_peaks(stim_time, post_stim)
data.filter_traces(lowpass_freq)
data.get_filtered_peaks(stim_time, post_stim)

data.get_filtered_peaks_kinetics(fs, stim_time, post_stim)
