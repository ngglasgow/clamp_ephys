...
paths = clamp_ephys.workflows.file_structure('server', 'Injected_GC_data_03262020/VC_pairs')
p2 = pd.DataFrame()
p14 = pd.DataFrame()

for root, dirs, files in os.walk(os.path.join(paths.tables, 'p2')):
    for name in files:
        if "mean_timeseries" in name:
            path = os.path.join(root, name)
            data = pd.read_csv(path)
            filename = '_'.join([name.split('_')[i] for i in [0, 1, 4]])
