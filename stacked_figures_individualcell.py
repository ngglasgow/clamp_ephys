...
data_dir = r'C:\Users\Jane Huang\Documents\Grad school\Urban Lab\Data Analysis\Injected_GC_data\New_VC_pairs\JH190828\slice_1\MC\data\not_analyzed'


# your file parsing loop that pulls out waves_mean and makes a waves_mean_df
# goes here

file_name = 'JH190828_c6_light100_gabazine_1.ibw'

cell_waves_df = igor_to_pandas(file_name, data_dir)

for i in range(len(cell_waves_df.columns)):
    plt.figure()
    plt.plot(cell_waves_df.iloc[:, i])
    
cell_waves_df = cell_waves_df.drop(columns=[4:])
cell_waves_df.drop(cell_waves_df.columns[4:], axis=1, inplace=True)
